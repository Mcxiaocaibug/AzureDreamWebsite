  ...
  onMount(() => {
    ...
    const updateScrollState = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const parallax = Math.min(scrollY, 720);

        root.style.setProperty("--page-progress", progress.toFixed(4));
        root.style.setProperty("--parallax", parallax.toFixed(1) + "px");
        /* 无单位版本供 opacity calc 使用（calc 中数字不能与长度做加减） */
        root.style.setProperty("--parallax-n", parallax.toFixed(1));
    ...
    const markSettled = (event) => {
      /* transitionend 会从子元素（.char / .panel-line 等）冒泡上来，只处理自身 */
      if (event.target !== event.currentTarget) return;
      if (event.propertyName === "opacity") {
        event.currentTarget.classList.add("is-settled");
        event.currentTarget.removeEventListener("transitionend", markSettled);
      }
    };
